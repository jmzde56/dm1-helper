...
async function analyzeFood() {
    ...
    if (apiK && apiK.length > 15) {
        try {
            ...
            alert("Error IA: " + e.message + "\n\nIntentando modo local...");
        }
    }

    // Caso 2: Fallback Local
    let totalCarbs = 0;
    let mainGI = 'Bajo';
    let found = false;
...
async function callGeminiAI(description, file, apiKey) {
    // Usamos el modelo más genérico y compatible
    const model = "gemini-1.5-flash";
    // Intentamos con v1 primero
    ...
    if (description && !description.includes("analizando")) {
        parts.push({ text: `Contexto: ${description}` });
    }
    ...
    parts.push({ text: "Analiza el plato. Estima Carbohidratos Netos (gramos) e Índice Glucémico (Alto/Medio/Bajo). Responde solo JSON: {\"carbs\": número, \"gi\": \"Alto/Medio/Bajo\", \"foods_detected\": [\"alimentos\"]}" });
    ...
    try {
        ...
        // Si v1 falla por "not found", intentamos v1beta como último recurso
        if (!response.ok && data.error?.message?.includes("not found")) {
            ...
            const bResponse = await fetch(BETA_URL, {
                method: 'POST',
                body: JSON.stringify({ contents: [{ parts }] })
            });
            data = await bResponse.json();
            if (!bResponse.ok) throw new Error(data.error?.message || "Fallo API Beta");
        } else if (!response.ok) {
