// DM1 Helper - Motor de Lógica v9.2 (Compatibilidad Total)
...
let USER_CONFIG = {
    ...
    XDRIP_URL: '',
    GEMINI_KEY: '',
    LAST_WORKING_MODEL: '' // Guardamos el modelo que funcione
};
...
function setupEventListeners() {
    ...
    fileInput.addEventListener('change', () => {
        if (fileInput.files.length > 0) {
            document.getElementById('food-description').value = "Foto recibida. Analizando...";
            analyzeFood();
        }
    });
...
async function testGeminiConnection() {
    ...
    if (!key) { alert("⚠️ Primero pega tu clave en Ajustes."); return; }

    const btn = document.getElementById('test-api-btn');
    btn.disabled = true;
    btn.textContent = "Buscando modelos compatibles...";

    try {
        const models = await listModels(key);
        if (models.length === 0) throw new Error("No se encontraron modelos disponibles para esta clave.");

        // Buscamos el mejor modelo disponible (2.0 flash > 1.5 flash > cualquier flash > gemini)
        const flashModel = models.find(m => m.name.includes('flash') && m.name.includes('2.0')) ||
        ...
        const modelId = flashModel.name.split('/').pop();

        btn.textContent = `Probando ${modelId}...`;
        await callGeminiAI("Hola, responde solo 'OK'", null, key, modelId);

        // Guardamos este modelo como el que funciona
        USER_CONFIG.LAST_WORKING_MODEL = modelId;
        localStorage.setItem('dm1_config', JSON.stringify(USER_CONFIG));

        alert("✅ ¡CONEXIÓN EXITOSA!\nModelo detectado: " + modelId + "\nYa puedes analizar tus platos.");
    } catch (e) {
...
async function getBestModel(apiKey) {
    ...
    if (USER_CONFIG.LAST_WORKING_MODEL) return USER_CONFIG.LAST_WORKING_MODEL;

    const models = await listModels(apiKey);
    const bestModel = models.find(m => m.name.includes('flash') && m.name.includes('2.0')) ||
        models.find(m => m.name.includes('flash') && m.name.includes('1.5')) ||
        models.find(m => m.name.includes('flash')) ||
        models.find(m => m.name.includes('gemini')) ||
        models[0];

    if (!bestModel) throw new Error("No hay un modelo compatible.");
...
async function analyzeFood() {
    ...
    if (apiK && apiK.length > 15) {
        try {
            ...
            alert("Error IA (" + APP_VERSION + "): " + e.message);
            // Si falló, borramos el modelo guardado para re-analizar la próxima vez
            USER_CONFIG.LAST_WORKING_MODEL = '';
...
function fallbackLocal(description, hasPhoto) {
    ...
    let mainGI = 'Bajo';
    let found = false;
    const cleanText = description.replace("foto recibida. analizando...", "").trim();
...
async function callGeminiAI(promptTxt, file, apiKey, modelId) {
    ...
    parts.push({ text: "Analiza nutricionalmente para DM1. Estima Carbohidratos Netos (g) e IG (Alto/Medio/Bajo). Responde ÚNICAMENTE en JSON: {\"carbs\": número, \"gi\": \"Alto/Medio/Bajo\", \"foods_detected\": [\"alimentos\"]}" });
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error?.message || "Fallo API Gemini");
    ...
    try {
        ...
        if (promptTxt.toLowerCase().includes("hola")) return { carbs: 0 };
...
async function fetchxDripGlucose() {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            if (document.getElementById('last-update')) document.getElementById('last-update').textContent = `Actualizado: ${time}`;
            updateDoseDisplay();
        }
