// DM1 Helper - Motor de Lógica v15.2 (Sport Intelligence & Expert System)
...
function setupEventListeners() {
    ...
    timerBtns.forEach(btn => {
        if (btn.id === 'exercise-advice-btn') return; // Skip advice btn
        btn.addEventListener('click', () => {
            const val = btn.getAttribute('data-hours');
            if (!val) return;
    ...
    // v15.2: Sport Hub Listeners
    const sportNavBtn = document.querySelector('[data-view="sport"]');
    if (sportNavBtn) {
        sportNavBtn.addEventListener('click', () => {
            document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
            document.getElementById('view-sport').classList.add('active');
            document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
            sportNavBtn.classList.add('active');
        });
    }

    const calcPrepBtn = document.getElementById('calc-prep-btn');
    if (calcPrepBtn) calcPrepBtn.addEventListener('click', calculateSportPreparation);

    const calcRecovBtn = document.getElementById('calc-recovery-btn');
    if (calcRecovBtn) calcRecovBtn.addEventListener('click', calculateSportRecovery);
}
...
async function getExerciseAdvice() {
    ...
    try {
        ...
        TAREA: Actúa como deportólogo experto. Indica si es seguro empezar, si requiere carbohidratos previos y da 3 ejemplos prácticos de comida (Rápido, Medio, Lento) según el estado actual. Si hay dudas sobre lo que tiene el paciente, pregúntale directamente. Sé breve y directo.`;
...
async function calculateSportPreparation() {
    const btn = document.getElementById('calc-prep-btn');
    const originalText = btn.textContent;
    const minToStart = document.getElementById('sport-pre-min').value || 30;
    const preType = document.getElementById('sport-pre-type').value;

    btn.disabled = true; btn.textContent = "Analizando Carga...";
    ...
    try {
        ...
        SITUACIÓN PRE-ENTRENO:
        - Salida en: ${minToStart} minutos.
        - Tipo de Ejercicio: ${preType}.
        - Glucemia: ${g} mg/dL.
        - Tendencia: ${trend}.
        - IOB (Insulina Activa): ${iob}.

        TAREA: Calcula cuántos gramos de Carbohidratos (CH) de navegación necesita EXACTAMENTE. 
        REGLA CRÍTICA: Si IOB > 0.5u, DEBE comer al menos 15-20g de CH combinados. 
        
        FORMATO:
        1. Indica el total de gramos.
        2. Ofrece 3 opciones prácticas (Fruta, Galletas, Pan, etc.). NO menciones manzanas con nueces salvo que el usuario diga que las tiene. Pregunta qué tiene disponible si prefieres.
        3. Explica el riesgo según el IOB y el tipo de ejercicio (${preType}).
        Sé quirúrgico y directo.`;

        const advice = await callGemini(prompt);
        openModal("🚀 Carga Pre-Entreno", advice);
    } catch (e) {
        alert("Error Prep: " + e.message);
    }
    btn.disabled = false; btn.textContent = originalText;
}

async function calculateSportRecovery() {
    const btn = document.getElementById('calc-recovery-btn');
    const originalText = btn.textContent;
    const activity = document.getElementById('sport-type-select').value;
    const dur = document.getElementById('sport-dur').value;
    const km = document.getElementById('sport-km').value;
    const hr = document.getElementById('sport-hr').value;

    btn.disabled = true; btn.textContent = "Analizando EPOC...";
    ...
    try {
        ...
        SITUACIÓN POST-ENTRENO (Finalizado ahora):
        - Actividad: ${activity}.
        - Volumen: ${km} km en ${dur} min.
        - Intensidad (Frecuencia Cardíaca): ${hr} LPM promedio.
        - Glucemia Actual: ${g} mg/dL (Tendencia: ${trend}).
        - IOB Actual: ${iob}.

        TAREA: Analiza la necesidad de recuperación enfocándote en la relación Intensidad/Volumen. 
        - Evalúa si los ${hr} LPM indican que fue una sesión predominantemente aeróbica (estabilidad) o anaeróbica (riesgo de caída tardía).
        - Si es MTB Anaeróbico o pulsaciones altas: Advierte sobre la caída de glucógeno y el peligro hipo en las próximas 4-8h.
        - Propón recuperación específica: CH rápidos para estabilizar tendencia ${trend} + CH lentos (Almidón resistente) para blindar el "Efecto Esponja" nocturno.
        - Indica gramos sugeridos y tipos de alimentos útiles.
        Sé quirúrgico y directo. No uses frases dulces.`;

        const advice = await callGemini(prompt);
        openModal("🔋 Recuperación Inteligente", advice);
    } catch (e) {
        alert("Error Recovery: " + e.message);
    }
    btn.disabled = false; btn.textContent = originalText;
}
