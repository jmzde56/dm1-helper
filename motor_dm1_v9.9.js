// DM1 Helper - Motor de Lógica v9.9 (Deep Diagnostics)
...
async function testKey(idInput, idBtn, label) {
    ...
    try {
        ...
        const result = await callGeminiAI("Saluda con la palabra HOLA", null, key, modelId);
        if (result) {
            alert(`✅ CLAVE ${label} EXITOSA\n\nConectado a: ${modelId}\nRespuesta recibida correctamente.`);
        } else {
            throw new Error("Respuesta vacía o mal estructurada.");
        }
    } catch (e) {
        console.error(e);
        alert(`❌ ERROR EN CLAVE ${label}:\n\n${e.message}\n\nDetalles técnicos: ${JSON.stringify(e.technical || 'Sin datos extra')}`);
    } finally {
        btn.disabled = false;
...
async function testGlucoseConnection() {
    ...
    try {
        ...
        alert("❌ Error: " + e.message + (location.protocol === 'https:' ? "\n\n⚠️ Nota: Estás en HTTPS, Juggluco debe ser HTTPS." : ""));
    } finally {
        btn.disabled = false;
...
function handleAIError(e, isBackup = false) {
    const keyLabel = isBackup ? "RESPALDO" : "PRINCIPAL";
    if (e.message.includes("quota")) {
        console.warn(`Límite excedido en clave ${keyLabel}.`);
        if (!isBackup && USER_CONFIG.GEMINI_KEY_BACKUP) return; // No alertamos si hay backup para probar
        alert(`⌛ CUOTA AGOTADA (${keyLabel}):\n${e.message}\n\nUsando base local.`);
    } else {
        alert(`⚠️ ERROR IA (${keyLabel}):\n${e.message}`);
    }
}
...
async function analyzeFood() {
    ...
    if (key1) {
        try {
            ...
            handleAIError(e, false);
        }
    }

    // 2. Intentamos Clave Respaldo si falló o no había clave 1
    if (!success && key2) {
        try {
...
function applyResults(result) {
    ...
    updateDoseDisplay();
    if (result.clarification_needed) setTimeout(() => alert("🙋 Pregunta: " + result.clarification_needed), 500);
}
...
function fallbackLocal(description, hasPhoto) {
    ...
    if (foundCount === 0) {
        if (hasPhoto) { totalCarbs = parseInt(prompt("No reconozco el plato en la foto. ¿Cuántos carbs estimas?", "15")) || 15; mainGI = "Medio"; }
        else if (cleanText.length > 2) { alert("No tengo '" + cleanText + "' en mi base local. Por favor edita los gramos manualmente."); totalCarbs = 0; }
...
async function callGeminiAI(promptTxt, file, apiKey, modelId) {
    ...
    parts.push({ text: "Analiza nutricionalmente para DM1. Estima gramos de Carbohidratos Netos (preciso) e Índice Glucémico (Alto/Medio/Bajo). Si es ambiguo, pregunta en 'clarification_needed'. Responde SOLO JSON: {\"carbs\": número, \"gi\": \"Alto/Medio/Bajo\", \"foods_detected\": [\"alimentos\"], \"clarification_needed\": \"pregunta opcional\"}" });
    ...
    const data = await response.json();
    if (!response.ok) {
        const err = new Error(data.error?.message || "Fallo en la comunicación con Google");
        err.technical = data.error;
        throw err;
    }
    ...
    try {
        ...
        const err = new Error("Error procesando respuesta JSON de la IA");
        err.technical = data;
        throw err;
    }
}
...
async function fetchGlucose() {
    ...
    } catch (e) { console.warn("Glucose fetch failed"); }
}
...
function saveSettings() {
    ...
    alert("✓ Ajustes guardados y verificados.");
    if (USER_CONFIG.XDRIP_URL) { fetchGlucose(); fetchTreatments(); }
}

function saveRecord() {
    const carbsValue = parseInt(document.getElementById('res-carbs').textContent);
    const record = {
        id: Date.now(),
        timestamp: Date.now(),
