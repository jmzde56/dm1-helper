// DM1 Helper - Motor de Lógica v14.0 (Advanced Safety & Navigation)
...
function saveRecord() {
    ...
    if (isPizza && fullDose > 0) {
        doseNow = fullDose / 2;
        alert(`🍕 EFECTO PIZZA\n\nInyecta ${doseNow} UI ahora.\nTe avisaré en 2 horas para inyectar las otras ${doseNow} UI.`);
        // Simulación de recordatorio diferido
        setTimeout(() => {
            alert(`🔔 RECORDATORIO: Inyectar 2da dosis (${doseNow} UI) para cubrir grasas/proteínas.`);
        }, 7200000); // 2 horas
    }
