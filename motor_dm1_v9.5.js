// DM1 Helper - Motor de Lógica v9.5 (Juggluco Support)
...
let USER_CONFIG = {
    ...
    ISF: 50,
    TARGET_GLUCOSE: 100,
    XDRIP_URL: 'http://127.0.0.1:17580', // Default para Juggluco/xDrip
    GEMINI_KEY: '',
    LAST_WORKING_MODEL: ''
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setupEventListeners();
    renderHistory();
    if (USER_CONFIG.XDRIP_URL) fetchGlucose(); // Función genérica para xDrip/Juggluco
});
...
function handleAIError(e) {
    if (e.message.includes("quota")) {
        alert("⌛ Cuota de IA agotada. Usando base de datos local.");
    } else {
        ...
        try {
            ...
            if (result && result.carbs !== undefined) {
                ...
                if (result.clarification_needed) {
                    setTimeout(() => alert("🙋 Pregunta de la IA: " + result.clarification_needed), 500);
                }
            }
        } catch (e) {
            handleAIError(e);
...
function fallbackLocal(description, hasPhoto) {
    ...
    if (foundCount === 0) {
        if (hasPhoto) {
            totalCarbs = parseInt(prompt("No reconozco el plato. ¿Cuántos carbohidratos crees que tiene?", "15")) || 15;
            mainGI = "Medio";
        } else if (cleanText.length > 3) {
            alert("No tengo '" + cleanText + "' en mi base local. Por favor, edita los carbs manualmente.");
            totalCarbs = 0;
        }
    } else {
        if (totalCarbs > 0 && totalCarbs < 10 && cleanText.includes("galletta")) {
            const cant = prompt("¿Cuántas unidades son?", "1");
...
async function callGeminiAI(promptTxt, file, apiKey, modelId) {
    ...
    parts.push({ text: "Analiza nutricionalmente para DM1. Estima gramos de Carbohidratos Netos (muy importante: sé preciso con porciones pequeñas) e Índice Glucémico (Alto/Medio/Bajo). Si dudas sobre la cantidad, pregunta en 'clarification_needed'. Responde SOLO JSON: {\"carbs\": número, \"gi\": \"Alto/Medio/Bajo\", \"foods_detected\": [\"alimentos\"], \"clarification_needed\": \"pregunta opcional\"}" });
...
async function fetchGlucose() {
    ...
    try {
        // Soporta xDrip+ y Juggluco Web Server
    ...
    } catch (e) { console.warn("Error de conexión con xDrip/Juggluco"); }
}
