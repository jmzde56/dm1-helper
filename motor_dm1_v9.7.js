// DM1 Helper - Motor de Lógica v9.7 (Sync Treatments)
    ...
    if (apiK && apiK.length > 15) {
        try {
            ...
            if (result && result.carbs !== undefined) {
                applyResults(result); success = true;
...
async function fetchTreatments() {
    if (!USER_CONFIG.XDRIP_URL) return;
    try {
        const resp = await fetch(`${USER_CONFIG.XDRIP_URL}/treatments.json?count=50`);
        const data = await resp.json();
        ...
        data.forEach(t => {
            ...
            if (tDate >= todayStart) {
                ...
                if (t.insulin) insulinToday += parseFloat(t.insulin);
            }

            // Simple IOB Calculation (4 hour decay for bolus)
            if (t.insulin && t.eventType !== 'Basal' && !t.notes?.toLowerCase().includes('tresiba')) {
                const ageMinutes = (now - tDate) / 60000;
                if (ageMinutes > 0 && ageMinutes < 240) {
                    iob += t.insulin * (1 - (ageMinutes / 240));
                }
            }
        });
    ...
    } catch (e) { console.warn("Treatments sync error"); }
}
