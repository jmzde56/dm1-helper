// DM1 Helper - Motor de Lógica v13.4 (Food Identification Fix)
...
async function fetchGlucose() {
    ...
    try {
        const r = await fetch(`${USER_CONFIG.XDRIP_URL}/sgv.json?count=288`);
    ...
    } catch (e) { console.error("Error Glucose:", e); }
}
...
async function fetchTreatments() {
    ...
    } catch (e) { console.error("Error Treatments:", e); }
}
...
async function analyzeFood(extraPrompt = "") {
    ...
    try {
        ...
        if (r.question) {
            ...
            if (r.food_name) desc.value = r.food_name; // RESTAURA NOMBRE
            applyResults(r);
        }
...
async function callGeminiAI(txt, file, key) {
    ...
    1. Identifica el nombre del plato brevemente en el campo 'food_name'.
    2. Si hay duda (cantidades o tipo de alimento), pregunta en 'question'. 
    Responde JSON: {"food_name": "nombre", "carbs": num, "gi": "Alto/Medio/Bajo", "question": "pregunta o null"}` });
...
function testKey(idIn, idBtn, lbl) { console.log("Test IA"); }
function testGlucoseConnection() { console.log("Test G"); }
function listPossibleModels() { console.log("List M"); }
