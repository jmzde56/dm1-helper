// DM1 Helper - Motor de Lógica v9.1 (Diagnóstico Pro)
...
async function testGeminiConnection() {
    ...
    try {
        // PASO 1: Listar modelos para ver qué tiene permitido esta clave
        const models = await listModels(key);
        if (models.length === 0) throw new Error("No se encontraron modelos disponibles para esta clave.");

        // PASO 2: Intentar un saludo con el primero que sea 'flash'
        const flashModel = models.find(m => m.name.includes('flash')) || models[0];
        ...
        alert("❌ FALLO DE CONEXIÓN:\n" + e.message + "\n\nCopia este error si necesitas ayuda.");
    } finally {
        btn.disabled = false;
...
async function analyzeFood() {
    ...
    if (apiK && apiK.length > 15) {
        try {
            // Buscamos dinámicamente el mejor modelo disponible
            const models = await listModels(apiK);
            const bestModel = models.find(m => m.name.includes('gemini-1.5-flash')) || models.find(m => m.name.includes('gemini-1.5'));

            if (!bestModel) throw new Error("No hay un modelo compatible con visión/flash.");
...
async function callGeminiAI(promptTxt, file, apiKey, modelId = "gemini-1.5-flash") {
    ...
    try {
        ...
        if (promptTxt.toLowerCase().includes("hola")) return { carbs: 0 }; // Caso de prueba
