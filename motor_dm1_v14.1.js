...
async function consultAICoach() {
    ...
    try {
        ...
        let promptText = `Eres un endocrinólogo y deportólogo experto en DM1. 
        Analiza estos datos (TIR ${glucoseStats.tir}%) y historial:\n${context}\n\n`;

        if (userQuestion) {
            promptText += `El paciente tiene esta duda específica: "${userQuestion}"\nResponde de forma clara, médica y motivadora.`;
        } else {
            promptText += `Realiza un análisis general de patrones de los últimos días y da consejos accionables.`;
        }
        ...
        const d = await r.json();
        if (!d.candidates) throw new Error("La IA no respondió. Revisa tu clave API.");
