...
async function consultAICoach() {
    ...
    try {
        ...
        // Solo enviamos los últimos 40 registros para no saturar
        const context = history.slice(0, 40).map(h => `${h.dateStr} ${h.date}: ${h.food} (${h.carbs}g HC, ${h.dose}U)`).join('\n');

        const prompt = `Eres un endocrinólogo experto en DM1 y deportología. 
        Analiza estos datos del paciente y busca patrones de hiper/hipoglucemia. 
        Contexto actual: TIR ${glucoseStats.tir}%, Bajo ${glucoseStats.low}%, Alto ${glucoseStats.high}%.
        Historial reciente:
        ${context}
        
        Da consejos accionables breves y motivadores sobre ratios o hábitos. Máximo 150 palabras.`;

        const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${USER_CONFIG.GEMINI_KEY}`, {
            method: 'POST',
        ...
        alert(`🩺 CONSEJO DEL COACH (IA)\n\n${advice}`);
    } catch (e) {
        alert("Error al consultar Coach: " + e.message);
    }
    btn.disabled = false; btn.textContent = "🩺 Consultar Coach";
}
