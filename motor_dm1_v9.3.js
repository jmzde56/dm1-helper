// DM1 Helper - Motor de Lógica v9.3 (Gestión de Cuota)
...
const FOOD_DATABASE = [
    { name: 'arroz', carbs: 28, gi: 'Alto' }, { name: 'pan', carbs: 50, gi: 'Alto' },
    { name: 'manzana', carbs: 14, gi: 'Bajo' }, { name: 'pasta', carbs: 25, gi: 'Medio' },
    { name: 'lentejas', carbs: 20, gi: 'Bajo' }, { name: 'pollo', carbs: 0, gi: 'Nulo' },
    { name: 'ensalada', carbs: 5, gi: 'Bajo' }, { name: 'pizza', carbs: 30, gi: 'Alto' },
    { name: 'empanada', carbs: 25, gi: 'Medio' }, { name: 'carne', carbs: 0, gi: 'Nulo' },
    { name: 'papa', carbs: 17, gi: 'Alto' }, { name: 'pure', carbs: 15, gi: 'Alto' },
    { name: 'milanesa', carbs: 15, gi: 'Medio' }, { name: 'hamburguesa', carbs: 30, gi: 'Medio' },
    { name: 'sanguche', carbs: 40, gi: 'Alto' }, { name: 'fruta', carbs: 15, gi: 'Bajo' },
...
async function testGeminiConnection() {
    ...
    try {
        const modelId = await getBestModel(key, true); // Forzamos búsqueda fresca
...
function handleAIError(e) {
    if (e.message.includes("quota")) {
        alert("⌛ ¡CUOTA EXCEDIDA!\n\nGoogle dice que has usado muchas veces la IA por hoy o muy rápido.\n\nSOLUCIÓN:\n1. Espera 1 minuto y vuelve a intentar.\n2. Si sigue fallando, Google reseteará tu cuota en unas horas.\n\n(La app usará el modo local mientras tanto)");
    } else {
...
async function getBestModel(apiKey, forceSearch = false) {
    ...
    if (!response.ok) throw new Error(data.error?.message || "Error al buscar modelos");
...
async function analyzeFood() {
    ...
    analyzeBtn.textContent = "Consultando...";
...
function fallbackLocal(description, hasPhoto) {
    let totalCarbs = 0; let mainGI = 'Bajo'; let found = false;
